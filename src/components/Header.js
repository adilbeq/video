...
const Header = (props) => {
    ...
    const sendServerVideo = () => {
        ...
        var videoData = document.getElementById("file").files[0];
        let modifedDateStart = state.data.startDate;
        modifedDateStart = modifedDateStart.split('T');
        let sendDateStart = modifedDateStart[0] + " " + modifedDateStart[1];
        console.log("splitted start: " + sendDateStart);

        let modifedDateEnd = state.data.endDate;
        modifedDateEnd = modifedDateEnd.split('T');
        let sendDateEnd = modifedDateEnd[0] + " " + modifedDateEnd[1];
        console.log("splitted end: " + sendDateEnd);
        
        formData.append('file', videoData);
        formData.append('ids', state.data.ids);
        formData.append('startDate', sendDateStart);
        formData.append('endDate', sendDateEnd);
        ...
        globalActions.uploadNewVideo(formData);
